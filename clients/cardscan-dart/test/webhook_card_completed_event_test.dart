import 'package:test/test.dart';
import 'package:cardscan_client/cardscan_client.dart';

// tests for WebhookCardCompletedEvent
void main() {
  final instance = WebhookCardCompletedEventBuilder();
  // TODO add properties to the builder and call build()

  group(WebhookCardCompletedEvent, () {
    // Unique identifier for the card.
    // String cardId
    test('to test the property `cardId`', () async {
      // TODO
    });

    // WebhookCardCompletedEventConfiguration configuration
    test('to test the property `configuration`', () async {
      // TODO
    });

    // Timestamp for when the card was created.
    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Flag indicating whether the card is deleted.
    // bool deleted
    test('to test the property `deleted`', () async {
      // TODO
    });

    // Unique identifier for the session.
    // String sessionId
    test('to test the property `sessionId`', () async {
      // TODO
    });

    // Type of event.
    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // Timestamp for the last update to the card record.
    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // Identifier for the user associated with the event.
    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

  });
}
