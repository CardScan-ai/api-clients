...
export const CardScanApiAxiosParamCreator = function (
  ...
  return {
    ...
    cardPerformance: async (
      ...
      const localVarPath = `/cards/{card_id}/performance`.replace(
        `{${"card_id"}}`,
  ...
  return {
    ...
    async cardPerformance(
      ...
      (
        axios?: AxiosInstance,
        basePath?: string,
      ) => AxiosPromise<CardPerformance200Response>
    > {
      const localVarAxiosArgs = await localVarAxiosParamCreator.cardPerformance(
        cardId,
        body,
        options,
      );
      ...
      const operationBasePath =
        operationServerMap["CardScanApi.cardPerformance"]?.[index]?.url;
    ...
    > {
      const localVarAxiosArgs = await localVarAxiosParamCreator.listEligibility(
        limit,
        cursor,
        options,
      );
    ...
    async setScanMetadata(
      ...
      (axios?: AxiosInstance, basePath?: string) => AxiosPromise<void>
    > {
      const localVarAxiosArgs = await localVarAxiosParamCreator.setScanMetadata(
        scanId,
        body,
        options,
      );
      const index = configuration?.serverIndex ?? 0;
      const operationBasePath =
        operationServerMap["CardScanApi.setScanMetadata"]?.[index]?.url;
      return (axios, basePath) =>
        createRequestFunction(
          localVarAxiosArgs,
          globalAxios,
...
export class CardScanApi extends BaseAPI {
  ...
  public withWebsocket = (
    cb: (
      ...
      error: WebSocket.ErrorEvent | null,
    ) => Promise<any>,
  ) => {
